...
class DOMjudgeTestGenerator:
    ...
    def __init__(self, problem_id, solution_file, generator_module=None):
        ...
        if solution_file.endswith(".cpp"):
            self.solution_type = "cpp"
            self.compile_solution()
        elif solution_file.endswith(".py"):
            self.solution_type = "python"
            # No compilation needed for Python
            print(f"Using Python solution file: {self.solution_file}")
        else:
            print(
                f"Error: Unsupported solution file format. Please use .cpp or .py files."
            )
            exit(1)
    ...
    def compile_solution(self):
        ...
        compiled_name = os.path.join(
            tempfile.gettempdir(), f"{self.problem_id}_solution"
        )
        try:
            subprocess.run(
    ...
    def generate_all_cases(self):
        ...
        if self.generator and hasattr(self.generator, "generate_all_cases"):
            ...
            print(
                "Warning: No custom generator provided. Using default test case generation."
            )
    ...
    def default_generate_case(self, case_num, params=None, is_sample=False):
        ...
        with open(input_file, "w") as f:
            ...
            f.write(
                f"You should implement a custom generator for problem {self.problem_id}.\n"
            )
    ...
    def generate_output_from_solution(self, input_file, output_file):
        ...
        try:
            with open(input_file, "r") as infile, open(output_file, "w") as outfile:
                if self.solution_type == "cpp":
                    # Run compiled C++ solution
                    subprocess.run(
                        [self.executable],
                        stdin=infile,
                        stdout=outfile,
                ...
                elif self.solution_type == "python":
                    # Run Python solution
                    subprocess.run(
                        [sys.executable, self.solution_file],
                        stdin=infile,
                        stdout=outfile,
                        check=True,
                        timeout=10,  # 10 second timeout to prevent hanging
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate DOMjudge-compatible test cases"
    )
    parser.add_argument("problem_id", type=str, help="Problem ID or name")
    parser.add_argument(
        "solution_file", type=str, help="Path to the solution file (C++ or Python)"
    )
    parser.add_argument(
        "--generator", type=str, help="Path to a custom generator module (Python)"
    )
