...
class Generator:
    ...
    def generate_all_cases(self, framework):
        ...
        # Sample test cases
        self.generate_case(
            framework, 1, {"n": 5, "q": 3, "max_val": 20, "min_val": 1}, is_sample=True
        )
        self.generate_case(
            framework,
            2,
            {"n": 10, "q": 5, "max_val": 100, "min_val": 1},
            is_sample=True,
        )

        # Corner cases
        self.generate_case(
            framework, 1, {"n": 1, "q": 1, "max_val": 10**9, "min_val": 10**9}
        )
        self.generate_case(framework, 2, {"n": 5, "q": 5, "max_val": 10, "min_val": 10})

        # Medium test cases
        self.generate_case(
            framework, 3, {"n": 100, "q": 100, "max_val": 10**5, "min_val": 1}
        )
        self.generate_case(
            framework, 4, {"n": 1000, "q": 1000, "max_val": 10**6, "min_val": 1}
        )

        # Large test cases
        self.generate_case(
            framework, 5, {"n": 10**5, "q": 10**5, "max_val": 10**9, "min_val": 1}
        )
        self.generate_case(
            framework,
            6,
            {"n": 2 * 10**5, "q": 2 * 10**5, "max_val": 10**9, "min_val": 1},
        )
        ...
        self.generate_case(
            framework, 10, {"n": 10**4, "q": 10**4, "max_val": 10**9, "min_val": 1}
        )
    ...
    def generate_case(self, framework, case_num, params, is_sample=False):
        ...
        with open(input_file, "w") as f:
            ...
            if pattern == "random":
                ...
                arr = sorted(
                    [random.randint(min_val, max_val) for _ in range(n)], reverse=True
                )
